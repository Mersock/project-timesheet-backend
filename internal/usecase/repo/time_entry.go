...
func (r *TimeEntryRepo) Insert(req request.CreateTimeEntryReq) (int64, error) {
	...
	sqlRaw := "INSERT INTO roles (status_id,work_type_id,user_id,,start_time,end_time,created_at) values (?,?,?,?,?,NOW()) "
	result, err := r.DB.Exec(sqlRaw, req.StatusID, req.WorkTypeID, req.UserID, req.StartDateTime, req.EndDateTime)
	if err != nil {
